...
const { tokenize_names } = require('../native/index.node');
...
function analyze(argv, callback) {
    ...
    if (argv.compare) {
        ...
        console.log('building collocation tables');
        buildCollocationTables(argv, pool, (err) =>{
            if (err) callback(err);
            console.log('built collocation tables');
            pool.end((err) => {
                if (err) callback(err);
                console.log('end');
...
function buildCollocationTables(argv, pool, cb) {
    ...
    extractTextField(type, limit, source, pool, (err, text) => {
        ...
        frequencyDistributionMunger(text, cc, (err, bigrams) => {
            if (err) cb(err);
            console.log('done with freq distribution');
...
function frequencyDistributionMunger(textfields, cc, callback) {
    ...
    const context = new Context({ country: cc, languages: ['ru'] });

    console.time('tokenize');
    const tokenized = tokenize_names(textfields, context);
    console.timeEnd('tokenize');
    for (const t of tokenized) {
        t.forEach((x) => {
            if (x.token_type) matchedTokens++;
            totalTokens++;
        });

        bigrams.update(t.map((x) => x.token));
    }
