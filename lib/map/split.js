...
const join = require('./interpolize').join;
...
class Split {
    ...
    split(feat, cb) {
        ...
        for (const cluster of clusters) {
            ...
            const ints = Split.genIntersectionFeature(cluster.intersectionPoints, feat.id);
            const pts = Split.genAddressPointsFeature(cluster.addressPoints);

            const result = Split.mergeFeatures(itps, pts, ints);
    ...
    /**
     * Break a cluster at provided locations
     *
     * @param {object} cluster
     * @param ... cluster.network
     * @param ... cluster.addressPoints,
     * @param ... cluster.intersectionPoints
     * @param {Number[]} breaks
     * @return {Object[]}
     */
    ...
    static splitSegments(cluster) {
        ...
        for (let i = 0; i < dists.length; i++) {
            ...
            // Review the addresses and attempt to match them to this segment. If we've already
            // considered all addresses, or don't have any points on a segment it won't be present
            // in the final output. We may want to reconsider this.
            while (addressIdx < cluster.addressPoints.length) {
                ...
                if (address.location >= start && address.location <= end) {
                    ...
                    if (addressIdx === cluster.addressPoints.length) {
                        ...
                        return segments; // All addresses have been reviewed.
                    }
                } else {
    ...
    static genIntersectionFeature(intersectionPoints, feature_id) {
        const coordinates = [];
        const properties = {
        ...
        for (let i = 0; i < intersectionPoints.length; i++) {
            if  (intersectionPoints[i].props.a_id === feature_id)  {
                properties['carmen:intersections'].push(intersectionPoints[i].props.b_street);
            } else if (intersectionPoints[i].props.b_id === feature_id) {
                properties['carmen:intersections'].push(intersectionPoints[i].props.a_street);
            } else {
                continue;
            }
    ...
    static genAddressPointsFeature(addressPoints) {
        const coordinates = [];
        const properties = {
     ...
     * @param {Object} intersection GeoJSON MultiPoint
     * @return {Object} GeoJSON GeometryCollection
     */
    static mergeFeatures(pts, itps, intersections) {
        ...
        for (const f of [pts, itps, intersections]) {
