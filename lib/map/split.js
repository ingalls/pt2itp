...
class Split {
    ...
    static splitCluster(cluster, breaks) {
        ...
        for (let i = 0; i < breaks.length; i++) {
            const isFirst = breaks[i - 1] === undefined;
            ...
            const slice = turf.lineSliceAlong(cluster.network, breakStartLocation, breakEndLocation);

            const intersectionPoints = cluster.intersectionPoints.filter((intsec) =>
                turf.pointOnLine(slice, intsec.coords).properties.dist === 0
            );
            ...
            res.push({
                network: slice,
                addressPoints: cluster.addressPoints.slice(addrStart, addrEnd),
                intersectionPoints
            });
            ...
            if (i === breaks.length - 1) {
                ...
                const finalSlice = turf.lineSliceAlong(cluster.network, finalBreakStart, finalBreakEnd);

                const finalIntersectionPoints = cluster.intersectionPoints.filter((intsec) =>
                    turf.pointOnLine(slice, intsec.coords).properties.dist === 0
                );

                res.push({
                    network: finalSlice,
                    addressPoints: cluster.addressPoints.slice(breaks[i] + 1),
