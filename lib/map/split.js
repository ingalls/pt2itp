...
class Split {
    ...
    split(feat, cb) {
        ...
        fs.appendFileSync('./out.txt', JSON.stringify(clusters) + '\n');
     ...
     * @param {Object} pts GeoJSON MultiPoint
     ...
     */
    static mergeFeatures(itps, pts, intersections) {
        ...
        if (pts.properties.address_props) feature.properties.address_props = pts.properties.address_props;

        if (itps.debug) feature.debug = itps.debug;
    ...
    static detectOutliers(cluster) {
        ...
        const locationRange = [];
        const coords = addressPoints.map((addr) => {
