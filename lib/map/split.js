...
class Split {
    ...
    split(feat, cb) {
        ...
        // TODO can we remove this as addresses now end up sorted by location on the line?
        ...
        const addressLocations = Split.attachPoints(network, coords, props);
        ...
        clusters = clusters.reduce((acc, cluster) => {
            const distDelta = cluster.addressPoints.reduce((m, v, i, a) => {
                if (i > 0) {
            ...
            acc.push(...Split.splitCluster(cluster, breaks).map(Split.detectOutliers));
        ...
        for (const cluster of clusters) {
            ...
            const itps = interpolize(Split.splitSegments(cluster), {
                debug: this.opts.debug,
                dropLow: !cluster.noDropLow,
                raiseHigh: !cluster.noRaiseHigh
            });

            if (itps.geometry.coordinates.length === 0) continue;

            let pts;
            if (cluster.addressPoints.length > 0) {
                pts = asGeoJSON.addressPointsFeature(cluster.addressPoints);
                if (pts.geometry.coordinates.length === 0) pts = undefined;
            }

            // Intersections can be disabled, we only add them to the output if requested.
            let ints;
            if (this.opts.intersections && cluster.intersectionPoints.length > 0) {
     ...
     *
     * @param {Object} network GeoJSON FeatureCollection of LineString features
    ...
    static splitCluster(cluster, breaks) {
        ...
        for (let i = 0; i < breaks.length; i++) {
            ...
            const addrStart = res.length === 0 ? 0 : breaks[i - 1] + 1;
            ...
            if (isLast) {
                ...
                if (finalBreakStart === finalBreakEnd) {
                    ...
                    res.push({
                        ...
                        breakStartLocation: finalBreakStart
                    });
                }
            }
        }
        ...
        return res.map((c, i) => {
            c.addressPoints = c.addressPoints.map((addr) => {
                addr.location = turf.pointOnLine(c.network, addr.coords).properties.location;
                return addr;
            });
            c.intersectionPoints = c.intersectionPoints.map((intsec) => {
                intsec.location = turf.pointOnLine(c.network, intsec.coords).properties.location;
                if (intsec.location > turf.length(c.network)) {
                    console.error('location: ' + intsec.location + ' length: ' + turf.length(c.network) + ' start: ' + c.breakStartLocation);
                }
                return intsec;
            });
            delete c.breakStartLocation;

            // Disable dropLow & RaiseHigh behavior.
            if (i === 0) c.noDropLow = true;
            if (i === res.length - 1) c.noRaiseHigh = true;
    ...
    /**
     * Detect outliers in a cluster by applying a linear regression
     * and removing points sufficiently far from the best fit line.
     *
     * @param {AddressCluster} cluster - address cluster containing address point data
     * @returns {AddressCluster}
     */
    static detectOutliers(cluster) {
        const addressPoints = cluster.addressPoints;
        // don't flag outliers when clusters have few points
        if (addressPoints.length < 5) return cluster;

        // count up and ignore evens/odds if there are less than five of either
        let evenCount = 0;
        let oddCount = 0;
        let ignoreEvens = false;
        let ignoreOdds = false;

        const locationRange = [];
        const coords = addressPoints.reduce((acc, addr) => {
            locationRange.push(addr.location);

            const number = parseInt(addr.props.number);
            ...
            acc.push([addr.location, number]);
            return acc;
        }, []);

        if (evenCount < 5) ignoreEvens = true;
        if (oddCount < 5) ignoreOdds = true;

        if (ignoreEvens && ignoreOdds) return cluster;

        const lowerTenth = ss.quantile(locationRange, 0.1);
        const upperTenth = ss.quantile(locationRange, 0.9);

        const fitLine = regression.linear(coords);
        const [slope, intercept] = fitLine.equation;

        // return if the correlation isn't strong enough
        if (fitLine.r2 < 0.3) return cluster;

        const distsFromFitLine = addressPoints.map((addr) => {
            const addrY = parseInt(addr.props.number);

            const lineY = slope * addr.location + intercept;

            addr.dist = lineY - addrY;
            return addr;
        });

        const bounds = Split.outlierBounds(distsFromFitLine.map((v) => v.dist));

        const flaggedOutliers = distsFromFitLine.map((addr) => {
            const number = parseInt(addr.props.number);
            if (isNaN(number)) {
                addr.ignore = true;
                return addr;
            ...
            if (ignoreOdds && number % 2 !== 0) return addr;
            if (ignoreEvens && number % 2 === 0) return addr;

            let mode = bounds.default;
            // apply relaxed bounds to the edges of the line to not aggressively
            // drop points for interpolation
            if (addr.location <= lowerTenth || addr.location >= upperTenth) {
                mode = bounds.relaxed;
            }

            if (addr.dist > mode.upper || addr.dist < mode.lower) {
                addr.ignore = true;
            }
            return addr;
        });
        cluster.addressPoints = flaggedOutliers;
        return cluster;
    }

    /**
     * Calculates the interquartile ranges for a list of numbers
     * and returns the lower and upper bounds to be used to flag outliers.
     *
     * Includes values for default outlier detection as well more relaxed
     * bounds.
     *
     * @param {Number[]} range - list of numbers to calculate bounds for
     * @returns {Object} - containing lower and upper bounds for each mode
     */
    static outlierBounds(range) {
        const firstQuartile = ss.quantile(range, 0.25);
        const thirdQuartile = ss.quantile(range, 0.75);

        const iqrRange = thirdQuartile - firstQuartile;

        const defaultRange = iqrRange * 1.5;
        const relaxedRange = iqrRange * 3.0;

        return {
            default: {
                lower: firstQuartile - defaultRange,
                upper: thirdQuartile + defaultRange
            },
            relaxed: {
                lower: firstQuartile - relaxedRange,
                upper: thirdQuartile + relaxedRange
            }
        };
    }
}
