...
class Split {
    ...
    split(feat, cb) {
        ...
        clusters = clusters.reduce((acc, cluster) => {
            ...
            acc.push(...Split.splitCluster(cluster, breaks));
            return acc;
        }, []);
     ...
     *
     * @param {AddressLocations[]} addressLocations - list of objects containing address point data
     * @returns {AddressLocations[]}
     */
    static detectOutliers(addressLocations) {
        // don't flag outliers when clusters have few points
        if (addressLocations.length < 5) return addressLocations;
        ...
        const locationRange = [];
        const coords = addressLocations.map((addr) => {
            locationRange.push(addr.location);

            const number = parseInt(addr.number);
            if (number % 2 === 0) {
                evenCount++;
            } else {
                oddCount++;
        ...
        if (ignoreEvens && ignoreOdds) return addressLocations;
        ...
        if (fitLine.r2 < 0.3) return addressLocations;

        const distsFromFitLine = addressLocations.map((addr) => {
            const addrY = parseInt(addr.number);
        ...
        const bounds = this.outlierBounds(distsFromFitLine.map((v) => v.dist));
