...
class Split {
    ...
    /**
     * Detect outliers in a cluster
     *
     * @param {AddressLocations[]} addressLocations
     * @returns {AddressLocations[]}
     */
    static detectOutliers(addressLocations) {
        if (addressLocations.length < 5) return addressLocations;

        const coords = addressLocations.map((addr) => [addr.location, parseInt(addr.number)]);

        const res = regression.linear(coords);
        const [slope, intercept] = res.equation;

        if (res.r2 < 0.3) return addressLocations;

        const distFromFitLine = addressLocations.map((addr) => {
        ...
        const sortedRange = distFromFitLine
            .map((v) => v.dist)
            .sort((a, b) => a - b);

        const { lower, upper } = this.range(sortedRange);

        const response = distFromFitLine.map((addr) => {
            if (upper > lower && (addr.dist > upper || addr.dist < lower) ||
                upper < lower && (addr.dist < upper || addr.dist > lower)) {
                addr.ignore = true;
            }
            return addr;
        });
    ...
    static range(input) {
        const firstQuartile = ss.quantile(input, 0.25);
        const thirdQuartile = ss.quantile(input, 0.75);

        const range = (thirdQuartile - firstQuartile) * 1.5;
        const lower = firstQuartile - range;
        const upper = thirdQuartile + range;
        return { lower, upper };
    }
}
