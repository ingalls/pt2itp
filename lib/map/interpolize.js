...
// TODO not sure about these two methods, as they have us iterating over each end of the array twice
// at least... and if ther are large number of nulls we may walk nearly the entire thing.
function firstNonNull(itps, prop) {
    for (let i = 0; i < itps.length; i++) {
        if (itps[i].properties[prop][0] !== null) return i;
    }
    return -1;
}
function lastNonNull(itps, prop) {
    for (let i = itps.length -1; i > -1; i--) {
        if (!itps[i].properties['carmen:rangetype']) continue;
        if (itps[i].properties[prop][0] !== null) return i;
    }
    return -1;
}
...
function itpSort(aFeat, bFeat) {
    let a, b;
    if (aFeat.properties['carmen:lfromhn'] && aFeat.properties['carmen:lfromhn'][0] !== undefined) {
        a = aFeat.properties['carmen:lfromhn'][0];
    } else if (aFeat.properties['carmen:rfromhn'] && aFeat.properties['carmen:rfromhn'][0] !== undefined) {
        a = aFeat.properties['carmen:rfromhn'][0];
    } else {
        return 1;
    }

    if (bFeat.properties['carmen:lfromhn'] && bFeat.properties['carmen:lfromhn'][0] !== undefined) {
        b = bFeat.properties['carmen:lfromhn'][0];
    } else if (bFeat.properties['carmen:rfromhn'] && bFeat.properties['carmen:rfromhn'][0] !== undefined) {
