...
module.exports.extendRange = extendRange;
...
function interpolize(splits, argv = {}) {
    ...
    const extendedItps = getExtendedRanges(itps, prevDelta, nextDelta, d);


    return join(extendedItps, argv);
}

/**
 * Creates a new range with the dropped/raised address number
 ...
 * @param  {Number} d         Difference of lower/upper bounds
 * @return {Object}           Array of interpolated addresses with extended dropped/raised ranges
 */
function getExtendedRanges(itps, prevDelta, nextDelta, d) {
    if (prevDelta === undefined) {
        // Drop Lower Values on L/R side to include more potential addresses ie 22 => 0
        const leftSide = buildExtRangeSide(itps, d, true, true);
        const rightSide = buildExtRangeSide(itps, d, true, false);
    ...
    if (nextDelta === undefined) {
        ...
        const leftSide = buildExtRangeSide(itps, d, false, true);
        const rightSide = buildExtRangeSide(itps, d, false, false);
 ...
 */
function buildExtRangeSide(itps, d, isPrev, isLeft) {
    let idx;
    ...
    if ((isPrev && lfrom < lto) || (!isPrev && lfrom > lto)) {
        const extNumber = isPrev ? dropLow(lfrom, d) : raiseHigh(lfrom, d);
        ...
        const extNumber = isPrev ? dropLow(lto, d) : raiseHigh(lto, d);
    ...
    if (range.properties[fromProp][0] !== undefined && range.properties[toProp][0] !== undefined) {
        ...
        range = extendRange(itps[idx], range, number, isPrev);
    } else return;
    if (!range) return;
 ...
 */
function extendRange(itp, range, number, isPrev) {
