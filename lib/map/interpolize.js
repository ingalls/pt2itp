...
function interpolize(splits, argv = {}) {
    ...
    for (const split of splits.segs) {
        ...
        if (result.rstart && result.rend) {
            if (parity.ro / parity.totalr > 0.70) result.parityr = 'O';
            if (parity.re / parity.totalr > 0.70) result.parityr = 'E';
        ...
        if (result.lstart && result.lend) {
            if (parity.lo / parity.totall > 0.70) result.parityl = 'O';
            if (parity.le / parity.totall > 0.70) result.parityl = 'E';
