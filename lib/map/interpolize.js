...
function buildExtRangeSide(itps, d, isPrev, isLeft) {
    ...
    if (range.properties[fromProp][0] !== undefined && range.properties[toProp][0] !== undefined) {
        ...
        range = extendRange(itps, range, idx, number, isPrev);
    } else return;
 ...
 * @param  {Number}  endIdx Position of the address number in itps
 * @param  {Number}  number Address number to drop/raise
 * @param  {Boolean} isPrev If the range is at the beggining of the segment or not.
 * @return {Object}         Extended range with extended geometry
 */
function extendRange(itps, range, endIdx, number, isPrev) {
    const addressnumbers = itps[endIdx].properties['carmen:addressnumber'][1];
    const numberIdx = isPrev ? addressnumbers.indexOf(number.toString()) : addressnumbers.lastIndexOf(number.toString());
