...
function addExtendedRange(itps, diff, isStart) {
    ...
    if (isSameIdx || leftSide) {
        const spliceIdx = isStart ? leftSide.idx : leftSide.idx + 1;
        let range = duplicateFeature(itps[leftSide.idx]);

        range = assingItpProperties(range, leftSide, isSameIdx ? rightSide : null);
        itps.splice(spliceIdx, 0, range);
    }

    if (!isSameIdx && rightSide) {
        const spliceIdx = isStart ? rightSide.idx : rightSide.idx + 1;
...
function buildExtSide(itps, diff, isStart, isLeft) {
    ...
    const isAscendent = (isStart && fromhn < tohn) || (!isStart && fromhn > tohn);

    // Do not raise/lower number if the segment's length is less than 10 meters
    const segment = getEdgeSegment(itp, isAscendent ? fromhn : tohn);
    if (!segment || turf.length(segment, { units: 'kilometers' }) <= 0.01) return;

    // Drop Lower/ Raise High Values on L/R side to include more potential addresses ie 22 => 0
    const extNumber = isStart ? dropLow(isAscendent ? fromhn : tohn, diff) : raiseHigh(isAscendent ? fromhn : tohn, diff);
    if (!extNumber || extNumber === (isAscendent ? fromhn : tohn)) return;

    sideOpt[fromProp] = isAscendent ? extNumber : tohn;
    sideOpt[toProp] = isAscendent ? fromhn : extNumber;
...
function getEdgeSegment(itp, number) {
    ...
    const start = turf.point(line.coordinates[rangeOpt.geometry.properties.start ? 0 : line.coordinates.length - 1]);
    return turf.lineSlice(start, rangeOpt.pointOnLine, line);
}

function assingItpProperties(itp, leftSide, rightSide) {
    const isBoth = leftSide && rightSide;

    itp.properties['carmen:lfromhn'][0] = isBoth || leftSide ? leftSide['carmen:lfromhn'] : null;
    itp.properties['carmen:ltohn'][0] = isBoth || leftSide ? leftSide['carmen:ltohn'] : null;
    itp.properties['carmen:rfromhn'][0] = isBoth || rightSide ? rightSide['carmen:rfromhn'] : null;
    itp.properties['carmen:rtohn'][0] = isBoth || rightSide ? rightSide['carmen:rtohn'] : null;
    itp.properties['carmen:parityl'][0] = isBoth || leftSide ? itp.properties['carmen:parityl'][0] : null;
    itp.properties['carmen:parityr'][0] = isBoth || rightSide ? itp.properties['carmen:parityr'][0] : null;
    itp.properties['carmen:addressnumber'] = [null,[]];
    itp.properties.address_props = [];
    itp.geometry.geometries[0].coordinates = isBoth || leftSide ? leftSide.segment.geometry.coordinates : rightSide.segment.geometry.coordinates;
    if (itp.geometry.geometries[1]) itp.geometry.geometries[1].coordinates = [];
    return itp;
}
