...
function buildExtRangeSide(itps, diff, isPrev, isLeft) {
    ...
    if (isPrev) {
        if (isLeft) idx = firstNonNull(itps, 'carmen:lfromhn');
        else idx = firstNonNull(itps, 'carmen:rfromhn');
    } else {
        if (isLeft) idx = lastNonNull(itps, 'carmen:lfromhn');
        else idx = lastNonNull(itps, 'carmen:rfromhn');
    }

    if (idx < 0) return; // No number to lower/raise number
    ...
    let range = duplicateFeature(itps[idx]);

    // If the number has been dropped or raised
    let hasNumberChanged = false;

    const lfrom = itps[idx].properties[fromProp][0];
    const lto = itps[idx].properties[toProp][0];
    let number;
    let isSmall;
    ...
    if ((isPrev && lfrom < lto) || (!isPrev && lfrom > lto)) {
        ...
        isSmall = isSmallRange(lfrom, extNumber);

        range.properties[fromProp][0] = assingExtendedNumber(lfrom, extNumber);
        // Don't create a new range, just lower/raise the original.
        range.properties[toProp][0] = isSmall ? lto : assignEdgeNumber(lfrom, extNumber);
        // If the range is small don't need the number information to create the segment geometry
        number = isSmall ? null : range.properties[toProp][0];
    } else {
        const extNumber = isPrev ? dropLow(lto, diff) : raiseHigh(lto, diff);

        // True if lowered/raised number diff from the original one by 10 or less
        isSmall = isSmallRange(lto, extNumber);

        range.properties[toProp][0] = assingExtendedNumber(lto, extNumber);
        // Don't create a new range, just lower/raise the original.
        range.properties[fromProp][0] = isSmall ? lfrom : assignEdgeNumber(lto, extNumber);
        // If the range is small don't need the number information to create the segment geometry
        number = isSmall ? null : range.properties[fromProp][0];
    }

    //  When the address number has been raised/dropped
    if (range.properties[fromProp][0] !== undefined && range.properties[toProp][0] !== undefined && !isSmall) {
        hasNumberChanged = true;
        range = buildExtRangeGeometry(itps[idx], range, number);
    }

    return { range, hasNumberChanged, idx, isSmall, number };
}
 ...
 */
function buildExtRangeGeometry(itp, range, number) {
    if (!itp || !itp.selectedRange) return range;
    ...
    // Delete point geometry from the new range segment geometry
    range.geometry.geometries[1].coordinates = [];

    const line = range.geometry.geometries[0]; // range segment geometry
    ...
    if (turf.length(lineFromEnd) > turf.length(lineFromStart)) line.coordinates = lineFromStart.geometry.coordinates;
    else line.coordinates = lineFromEnd.geometry.coordinates;

    range.geometry.geometries[0] = line;
...
function buildExtRangeEnd(itps, leftSide, rightSide, isPrev) {
    ...
    if (leftSide) spliceIdx = isPrev ? leftSide.idx : leftSide.idx + 1;

    if (leftSide && rightSide && (leftSide.hasNumberChanged || rightSide.hasNumberChanged) && leftSide.idx === rightSide.idx && leftSide.range.extended) {
        leftSide.range.properties['carmen:rtohn'][0] = rightSide.range.properties['carmen:rtohn'][0];
        leftSide.range.properties['carmen:rfromhn'][0] = rightSide.range.properties['carmen:rfromhn'][0];

        itps.splice(spliceIdx, 0, leftSide.range);

    } else if (leftSide && rightSide && (leftSide.isSmall || !leftSide.range.extended)){
        itps[leftSide.idx] = assingItpProperties(itps[leftSide.idx], leftSide, rightSide);

    } else if (!leftSide || !rightSide || leftSide.idx !== rightSide.idx) {
        if (leftSide && leftSide.hasNumberChanged) {
            leftSide.range.properties['carmen:rtohn'][0] = null;
            leftSide.range.properties['carmen:rfromhn'][0] = null;
            leftSide.range.properties['carmen:parityr'][0] = null;

            itps.splice(spliceIdx, 0, leftSide.range);
        } else if (leftSide && (leftSide.isSmall || !leftSide.range.extended)) itps[leftSide.idx] = assingItpProperties(itps[leftSide.idx], leftSide, null);

        if (rightSide && rightSide.hasNumberChanged) {
            spliceIdx = isPrev ? rightSide.idx : rightSide.idx + 1;
            rightSide.range.properties['carmen:ltohn'][0] = null;
            rightSide.range.properties['carmen:lfromhn'][0] = null;
            rightSide.range.properties['carmen:parityl'][0] = null;

            itps.splice(spliceIdx, 0, rightSide.range);
        } else if (rightSide && (rightSide.isSmall || !rightSide.range.extended)) itps[rightSide.idx] = assingItpProperties(itps[rightSide.idx], null, rightSide);
    }
...
function assingItpProperties(itp, leftSide, rightSide) {
    if (leftSide) {
        itp.properties['carmen:lfromhn'][0] = leftSide.range.properties['carmen:lfromhn'][0];
        itp.properties['carmen:ltohn'][0] = leftSide.range.properties['carmen:ltohn'][0];
    }

    if (rightSide) {
        itp.properties['carmen:rfromhn'][0] = rightSide.range.properties['carmen:rfromhn'][0];
        itp.properties['carmen:rtohn'][0] = rightSide.range.properties['carmen:rtohn'][0];
    }
