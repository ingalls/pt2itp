...
function getExtendedRanges(itps, prevDelta, nextDelta, d) {
    if (prevDelta === undefined) {
        ...
        itps = buildExtRange(itps, leftSide, rightSide, true);
    }
    ...
    if (nextDelta === undefined) {
        ...
        itps = buildExtRange(itps, leftSide, rightSide, false);
    }
    return itps;
}
 ...
 * @return {Object}         It conrains range, hasNumberChanged and index in the array
 */
function buildExtRangeSide(itps, d, isPrev, isLeft) {
    ...
    if (idx >= 0) {
        ...
        if ((isPrev && lfrom < lto) || (!isPrev && lfrom > lto)) {
            const change = isPrev ? dropLow(lfrom, d) : raiseHigh(lfrom, d);
            range.properties[fromProp][0] = assingExtendedNumber(lfrom, change);
            range.properties[toProp][0] = assignEdgeNumber(lfrom, change);
            number = range.properties[toProp][0];
        } else {
            const change = isPrev ? dropLow(lto, d) : raiseHigh(lto, d);
            range.properties[toProp][0] = assingExtendedNumber(lto, change);
            range.properties[fromProp][0] = assignEdgeNumber(lto, change);
            number = range.properties[fromProp][0];
        }

        if (range.properties[fromProp][0] && range.properties[toProp][0]) {
...
/**
 * Returns the dropped/raised number if the oriinal number has changed.
...
function extendRange(itps, range, endIdx, number, isPrev) {
    ...
    range.properties['carmen:addressnumber'] = [null, [number.toString()]];
    ...
    for (let i = 0; i < range.geometry.geometries.length; i++) {
        if (range.geometry.geometries[i].type === 'MultiPoint') {
            ...
            range.geometry.geometries[i].coordinates = [numberGeom];
        }
    }
    ...
    for (let i = 0; i < range.geometry.geometries.length; i++) {
        if (range.geometry.geometries[i].type === 'LineString') {
            const startIdx = isPrev ? 0 : range.geometry.geometries[i].coordinates.length - 1;
            const start = turf.point(range.geometry.geometries[i].coordinates[startIdx]);
            const stop = turf.point(numberGeom);
            const line = turf.lineSlice(start, stop, range.geometry.geometries[i]);
            range.geometry.geometries[i].coordinates = line.geometry.coordinates;
        }
    }
    return range;
}
 ...
 */
function buildExtRange(itps, leftSide, rightSide, isPrev) {
    let spliceIdx = isPrev ? leftSide.idx : leftSide.idx + 1;

    if ((leftSide.hasNumberChanged || rightSide.hasNumberChanged) && leftSide.idx === rightSide.idx) {
        ...
        leftSide.range.properties['carmen:addressnumber'][1].push(rightSide.range.properties['carmen:addressnumber'][1][0]);

        for (let i = 0; i < leftSide.range.geometry.geometries.length; i++) {
            if (leftSide.range.geometry.geometries[i].type === 'MultiPoint') {
                for (let j = 0; j < rightSide.range.geometry.geometries.length; j++) {
                    const rgeom = rightSide.range.geometry.geometries[j].coordinates[0];
                    if (rightSide.range.geometry.geometries[j].type === 'MultiPoint') leftSide.range.geometry.geometries[i].coordinates.push(rgeom);
                }
            }
        }
...
function lsb(start, end) {
    return misc.sign(misc.det2D(
        ...
        turf.destination(
            ...
            0.01,
            turf.bearing(turf.point(start), turf.point(end)) - 90, { units: 'miles' }).geometry.coordinates
    ));
}
