...
function buildExtSide(itps, diff, isStart, isLeft) {
    ...
    const fromhn = itp.range[fromProp].number;
    const tohn = itp.range[toProp].number;
...
function generateRange(itp, leftSide, rightSide) {
    ...
    return {
        range: {
            parityl: isBoth || leftSide ? itp.parityl : null,
            lstart: isBoth || leftSide ? itp.lstart : null,
            lend:  isBoth || leftSide ? itp.lend : null,
            parityr: isBoth || rightSide ? itp.parityr : null,
            rstart: isBoth || rightSide ? itp.rstart : null,
            rend: isBoth || rightSide ? itp.rend : null
        },
 ...
 * @property {Number|Null} lstart
 * @property {Number|Null} lend
 * @property {String|Null} parityr - "O" or "E" for odd / even
 * @property {Number|Null} rstart
 * @property {Number|Null} rend
 */
 ...
 * @param {InterpolationRange} aRange
 * @param {InterpolationRange} bRange
 * @return {Number}
 */
function itpSort(aRange, bRange) {
    let a = 1;
    let b = 1;

    if (aRange.lstart) {
        a = aRange.lstart;
    } else if (aRange.rstart) {
        a = aRange.rstart;
    }

    if (bRange.lstart) {
        b = bRange.lstart;
    } else if (bRange.rstart) {
        b = bRange.rstart;
    }
