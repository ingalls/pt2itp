...
module.exports.buildExtRangeGeometry = buildExtRangeGeometry;
...
function interpolize(splits, argv = {}) {
    ...
    itps = addExtendedRanges(itps, prevDelta, nextDelta, d);

    return join(itps, argv);
}

/**
 * Creates a new range with the dropped/raised address number and add it to the end.
 * @param  {Object} itps      Array of interpolated addresses
 * @param  {Number} prevDelta Address number difference from previous cluster
 * @param  {Number} nextDelta Address number difference to next cluster
 * @param  {Number} diff         Difference of lower/upper bounds
 * @return {Object}           Array of interpolated addresses with extended dropped/raised ranges
 */
function addExtendedRanges(itps, prevDelta, nextDelta, diff) {
    ...
    if (prevDelta === undefined) {
        ...
        const leftSide = buildExtRangeSide(itps, diff, true, true);
        const rightSide = buildExtRangeSide(itps, diff, true, false);

        // Ignore if there is no L/R number to lower.
        if (leftSide || rightSide) itps = buildExtRangeEnd(itps, leftSide, rightSide, true);
    }

    if (nextDelta === undefined) {
        // Add range at the beginnig with the L/R raised number.
        // Raise Upper Values on L/R side
        const leftSide = buildExtRangeSide(itps, diff, false, true);
        const rightSide = buildExtRangeSide(itps, diff, false, false);

        // Ignore if there is no L/R number to raise.
        if (leftSide || rightSide) itps = buildExtRangeEnd(itps, leftSide, rightSide, false);
    }
...
/**
 * Generate lfromhn and ltohn/rfromhn and rtohn properties with
 * the dropped/raised numbers.
 * @param  {Object}  itps   Array of interpolated addresses
 * @param  {Number}  diff   Difference of lower/upper bounds
 * @param  {Boolean} isPrev If the range is at the beggining of the segment or not.
 * @param  {Boolean} isLeft If it is building the left or right side.
 * @return {Object}         It conrains range, hasNumberChanged and index in the array. I return null if there is no firstnull or lastnull
 */
function buildExtRangeSide(itps, diff, isPrev, isLeft) {
    const sideOpt = {
        idx: null, // Index in itps
        hasNumberChanged: false, // Number has been dropped/raised
        isSmall: false, // Diff between number and its dropped/raised version is less than 10
        number: null, // Original number dropped/raised
        segment: null, // Coordinates of new interpolation range segment
        isLeft: isLeft
    };

    // Get the index of the the first/last nonull address number
    if (isPrev) {
        if (isLeft) sideOpt.idx = firstNonNull(itps, 'carmen:lfromhn');
        else sideOpt.idx = firstNonNull(itps, 'carmen:rfromhn');
    } else {
        if (isLeft) sideOpt.idx = lastNonNull(itps, 'carmen:lfromhn');
        else sideOpt.idx = lastNonNull(itps, 'carmen:rfromhn');
    }
    sideOpt.idx = sideOpt.idx < 0 ? null : sideOpt.idx;

    if (sideOpt.idx === null) return sideOpt; // No number to lower/raise number

    let fromProp;
    let toProp;

    if (isLeft) {
        // Set properties for the left side
        fromProp = 'carmen:lfromhn';
        toProp = 'carmen:ltohn';
    } else {
        // Set properties for the right side
        fromProp = 'carmen:rfromhn';
        toProp = 'carmen:rtohn';
    }
    console.log(itps[sideOpt.idx].selectedRange)
    const lfrom = itps[sideOpt.idx].properties[fromProp][0];
    const lto = itps[sideOpt.idx].properties[toProp][0];

    console.log('lfrom',lfrom,'lto',lto)

    if ((isPrev && lfrom < lto) || (!isPrev && lfrom > lto)) {
        const extNumber = isPrev ? dropLow(lfrom, diff) : raiseHigh(lfrom, diff);

        // True if lowered/raised number diff from the original one by 10 or less
        sideOpt.isSmall = isSmallRange(lfrom, extNumber);
        sideOpt[fromProp] = assingExtendedNumber(lfrom, extNumber);
        sideOpt[toProp] = sideOpt.isSmall ? lto : assignEdgeNumber(lfrom, extNumber);
        console.log('sideOpt[toProp]',sideOpt[toProp])

        sideOpt.number = sideOpt.isSmall ? null : sideOpt[toProp];
    } else {
        const extNumber = isPrev ? dropLow(lto, diff) : raiseHigh(lto, diff);

        // True if lowered/raised number diff from the original one by 10 or less
        sideOpt.isSmall = isSmallRange(lto, extNumber);
        sideOpt[toProp] = assingExtendedNumber(lto, extNumber);
        sideOpt[fromProp] = sideOpt.isSmall ? lfrom : assignEdgeNumber(lto, extNumber);
         console.log('sideOpt[fromProp]',sideOpt[fromProp])

        // If the range is small don't need the number information to create the segment geometry
        sideOpt.number = sideOpt.isSmall ? null : sideOpt[fromProp];
    }

    //  When the address number has been raised/dropped
    if (sideOpt[fromProp] !== undefined && sideOpt[toProp] !== undefined && !sideOpt.isSmall) {
        sideOpt.hasNumberChanged = true;
        const line = itps[sideOpt.idx].geometry.geometries[0]; // range segment geometry

        sideOpt.segment = buildExtRangeGeometry(itps[sideOpt.idx], line, sideOpt.number);
    }
...
/**
 * Returns the dropped/raised number if the original number has changed.
 * @param  {number} oldNumber original address number to be dropped/raised.
 * @param  {number} newNumber dropped/raised address number
 * @return {number}           undefined if the original number didn't change
 */
function assingExtendedNumber(oldNumber, newNumber) {
    if (oldNumber === newNumber) return;
    return newNumber;
}

/**
 * Returns the original number if it has been dropped/raised to build
 * the opposite end of the new  extended interpolation range
 * @param  {number} oldNumber original address number
 * @param  {number} newNumber dropped/raised address number
 * @return {number}           undefined if the original address number didn't change.
 */
function assignEdgeNumber(oldNumber, newNumber) {
    if (oldNumber === newNumber) return;
    return oldNumber;
}

function isSmallRange(oldNumber, newNumber) {
    return Math.abs(oldNumber - newNumber) <= 10 ? true : false;
}

/**
 * Creates the segment geometry for the extended range generated with the raised and lowered numbers.
 * @param  {Object}  itp    Interpolated addresses
 * @param  {Object}  range  Extended range
 * @param  {Number}  number Address number to drop/raise
 * @return {Object}         Extended range with extended geometry
 */
function buildExtRangeGeometry(itp, line, number) {
    // console.log(JSON.stringify(itp))
    // console.log(JSON.stringify(line))
    // console.log(number)
    if (!itp || !itp.selectedRange) return;
    let segment;

    const selRange = itp.selectedRange;
    let side;

    if (selRange.lstart && selRange.lstart.number === number) side = selRange.lstart;
    else if (selRange.lend && selRange.lend.number === number) side = selRange.lend;
    else if (selRange.rstart && selRange.rstart.number === number) side = selRange.rstart;
    else if (selRange.rend && selRange.rend.number === number) side = selRange.rend;

    if (!side) return;

    const stop = side.pointOnLine;

    // calculate segment from start
    const startFromStart = turf.point(line.coordinates[0]);
    const lineFromStart = turf.lineSlice(startFromStart, stop, line);

    // Calculate segment from end
    const startFromEnd = turf.point(line.coordinates[line.coordinates.length - 1]);
    const lineFromEnd = turf.lineSlice(startFromEnd, stop, line);

    if (turf.length(lineFromEnd) > turf.length(lineFromStart)) segment = lineFromStart.geometry.coordinates;
    else segment = lineFromEnd.geometry.coordinates;

    return segment;
}

/**
 * Creates ranges at the start and end of the geometry
 * according to the dropped and raised numbers
 * @param  {Object}  itps      Array of interpolated addresses
 * @param  {Object}  leftSide  dropped/raised lfromhn and ltohn
 * @param  {Object}  rightSide dropped/raised rfromhn and rtohn
 * @param  {Boolean} isPrev    If the range is at the beggining of the segment or not.
 * @return {Object}            Array of interpolated addresses
 */
function buildExtRangeEnd(itps, leftSide, rightSide, isPrev) {
    let spliceIdx;
    if (leftSide.idx) spliceIdx = isPrev ? leftSide.idx : leftSide.idx + 1;
    // console.log('itps[leftSide.idx]')
    // console.log(JSON.stringify(itps[leftSide.idx]))

    if (leftSide.idx === null && rightSide.idx === null) return itps
    else if (leftSide.idx!==null && rightSide.idx!==null) {
        if (leftSide.isSmall || rightSide.isSmall) {
            // Update the range with the raised/lowered number
            itps[leftSide.idx] = assingItpProperties(itps[leftSide.idx], leftSide, rightSide);            
        } else {
            // Add a new range
            let range = duplicateFeature(itps[leftSide.idx]);
            range = assingItpProperties(range, leftSide, rightSide); 
            range.geometry.geometries[0].coordinates = leftSide.segment;
            range.geometry.geometries[1].coordinates = [];

            itps.splice(spliceIdx, 0, range);
        }
    } else {
        if (leftSide.idx!==null) {
            if (leftSide.isSmall) {
                itps[leftSide.idx] = assingItpProperties(itps[leftSide.idx], leftSide, null);
            } else {
                let range = duplicateFeature(itps[leftSide.idx]);
                range = assingItpProperties(range, leftSide, null);
                range.properties['carmen:rtohn'][0] = null;
                range.properties['carmen:rfromhn'][0] = null;
                range.properties['carmen:parityr'][0] = null;
                range.geometry.geometries[0].coordinates = leftSide.segment;
                range.geometry.geometries[1].coordinates = [];
        ...
        if (rightSide.idx !== null) {
            if (rightSide.isSmall) {
                itps[rightSide.idx] = assingItpProperties(itps[rightSide.idx], null, rightSide);
            } else {
                spliceIdx = isPrev ? rightSide.idx : rightSide.idx + 1;
                let range = duplicateFeature(itps[rightSide.idx]);
                range = assingItpProperties(range, null, rightSide);
                range.properties['carmen:ltohn'][0] = null;
                range.properties['carmen:lfromhn'][0] = null;
                range.properties['carmen:parityl'][0] = null;
                range.geometry.geometries[0].coordinates = leftSide.segment;
                range.geometry.geometries[1].coordinates = [];
...
function assingItpProperties(itp, leftSide, rightSide) {
    if (leftSide) {
        itp.properties['carmen:lfromhn'][0] = leftSide['carmen:lfromhn'];
        itp.properties['carmen:ltohn'][0] = leftSide['carmen:ltohn'];
    }

    if (rightSide) {
        itp.properties['carmen:rfromhn'][0] = rightSide['carmen:rfromhn'];
        itp.properties['carmen:rtohn'][0] = rightSide['carmen:rtohn'];
    }
...
/**
 * Creates a new range based on the first/last range
 * @param  {Object} feat Address feature
 * @return {Object}      Address feature
 */
function duplicateFeature(feat) {
    const feature = JSON.parse(JSON.stringify(feat));

    // delete feature.rangeOptions;
    delete feature.genFeatOptions;
    feature.properties.address_props = [];
    feature.properties['carmen:addressnumber'] = [null,[]];
    return feature;
}
