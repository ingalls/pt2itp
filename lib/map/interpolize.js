 ...
 * @param {boolean} argv.dropLow Round the lowest range down
 * @param {boolean} argv.raiseHigh Round the highest range up
...
function calculateInterpolationParams(split, limits) {
    ...
    for (let i = 0; i < split.addressPoints.length; i++) {
        // Ignore flagged outliers
        if (split.addressPoints[i].props.ignore) continue;
