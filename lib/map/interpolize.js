...
function interpolize(splits, argv = {}) {
    ...
    for (const split of splits.segs) {
        ...
        if (!split.address || !split.number) {
            ...
            // TODO why do we generate this feature if we don't have interpolation data?
...
function calculateInterpolationParams(split, limits) {
    ...
    for (const res of dist) {
        if (res.distFromLine < distFromLineLimit) {
            if (parseInt(res.number) < limits.min) limits.min = parseInt(res.number);
            if (parseInt(res.number) > limits.max) limits.max = parseInt(res.number);
