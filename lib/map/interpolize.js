...
function getExtendedRanges(itps, prevDelta, nextDelta, d) {
    if (prevDelta === undefined) {
        ...
        let ldropRange = duplicateFeature(itps[leftIdx]);
        let ladd = false;

        if (leftIdx >= 0) {
            const lfrom = itps[leftIdx].properties['carmen:lfromhn'][0];
            const lto = itps[leftIdx].properties['carmen:ltohn'][0];
            let number;

            if (lfrom < lto) {
                const drop = dropLow(lfrom, d);
                ldropRange.properties['carmen:lfromhn'][0] = lfrom === drop ? null : drop;
                ldropRange.properties['carmen:ltohn'][0] = lfrom === drop ? null : lfrom;
                number = ldropRange.properties['carmen:ltohn'][0];
            } else {
                const drop = dropLow(lto, d);
                ldropRange.properties['carmen:ltohn'][0] = lto === drop ? null : drop;
                ldropRange.properties['carmen:lfromhn'][0] = lto === drop ? null : lto;
                number = ldropRange.properties['carmen:lfromhn'][0];
            }

            if (ldropRange.properties['carmen:lfromhn'][0] && ldropRange.properties['carmen:ltohn'][0]) {
                ladd = true;
                ldropRange = extendRange(itps, ldropRange, leftIdx, number, true);
            }
        }

        const rightIdx = firstNonNull(itps, 'carmen:rfromhn');

        let rdropRange = duplicateFeature(itps[rightIdx]);
        let radd = false;
        let number;

        if (rightIdx >= 0) {
            const rfrom = itps[rightIdx].properties['carmen:rfromhn'][0];
            const rto = itps[rightIdx].properties['carmen:rtohn'][0];

            if (rfrom < rto) {
                const drop = dropLow(rfrom, d);
                rdropRange.properties['carmen:rfromhn'][0] = rfrom === drop ? null : drop;
                rdropRange.properties['carmen:rtohn'][0] = rfrom === drop ? null : rfrom;
                number = rdropRange.properties['carmen:rtohn'][0];
            } else {
                const drop = dropLow(rto, d);
                rdropRange.properties['carmen:rtohn'][0] = rto === drop ? null : drop;
                rdropRange.properties['carmen:rfromhn'][0] = rto === drop ? null : rto;
                number = rdropRange.properties['carmen:rfromhn'][0];
            }

            if (rdropRange.properties['carmen:rfromhn'][0] && rdropRange.properties['carmen:rtohn'][0]) {
                radd = true;
                rdropRange = extendRange(itps, rdropRange, rightIdx, number, true);
            }
        }

        itps = buildExtendedRange(itps, ladd, radd, leftIdx, rightIdx, ldropRange, rdropRange, true);
    }
    ...
    if (nextDelta === undefined) {
        ...
        let lraiseRange = duplicateFeature(itps[leftEnd]);
        let ladd = false;

        if (leftEnd >= 0) {
            const lfrom = itps[leftEnd].properties['carmen:lfromhn'][0];
            const lto = itps[leftEnd].properties['carmen:ltohn'][0];
            let number;

            if (lfrom > lto) {
                const raise = raiseHigh(lfrom, d);
                lraiseRange.properties['carmen:lfromhn'][0] = lfrom === raise ? null : raise;
                lraiseRange.properties['carmen:ltohn'][0] = lfrom === raise ? null : lfrom;
                number = lraiseRange.properties['carmen:ltohn'][0];
            } else {
                const raise = raiseHigh(lto, d);
                lraiseRange.properties['carmen:ltohn'][0] = lto === raise ? null : raise;
                lraiseRange.properties['carmen:lfromhn'][0] = lto === raise ? null : lto;
                number = lraiseRange.properties['carmen:lfromhn'][0];
            }
            if (lraiseRange.properties['carmen:lfromhn'][0] && lraiseRange.properties['carmen:ltohn'][0]) {
                lraiseRange = extendRange(itps, lraiseRange, leftEnd, number, false);
                ladd = true;
            }
        ...
        let rraiseRange = duplicateFeature(itps[rightEnd]);

        let radd = false;

        if (rightEnd >= 0) {
            const rfrom = itps[rightEnd].properties['carmen:rfromhn'][0];
            const rto = itps[rightEnd].properties['carmen:rtohn'][0];
            let number;

            if (rfrom > rto) {
                const raise = raiseHigh(rfrom, d);
                rraiseRange.properties['carmen:rfromhn'][0] = rfrom === raise ? null : raise;
                rraiseRange.properties['carmen:rtohn'][0] = rfrom === raise ? null : rfrom;
                number = rraiseRange.properties['carmen:rtohn'][0];
            } else {
                const raise = raiseHigh(rto, d);
                rraiseRange.properties['carmen:rtohn'][0] = rto === raise ? null : raise;
                rraiseRange.properties['carmen:rfromhn'][0] = rto === raise ? null : rto;
                number = rraiseRange.properties['carmen:rfromhn'][0];
            }

            if (rraiseRange.properties['carmen:rfromhn'][0] && rraiseRange.properties['carmen:rtohn'][0]) {
                rraiseRange = extendRange(itps, rraiseRange, rightEnd, number, false);
                radd = true;
            }
        }

        itps = buildExtendedRange(itps, ladd, radd, leftEnd, rightEnd, lraiseRange, rraiseRange, false);
    }
...
function buildExtendedRange(itps, ladd, radd, leftEnd, rightEnd, lRange, rRange, isStart) {
    let spliceIdx = isStart ? leftEnd : leftEnd + 1;

    if ((ladd || radd) && leftEnd === rightEnd) {
        lRange.properties['carmen:rtohn'][0] = rRange.properties['carmen:rtohn'][0];
        lRange.properties['carmen:rfromhn'][0] = rRange.properties['carmen:rfromhn'][0];
        lRange.properties['carmen:addressnumber'][1].push(rRange.properties['carmen:addressnumber'][1][0]);

        for (let i = 0; i < lRange.geometry.geometries.length; i++) {
            if (lRange.geometry.geometries[i].type === 'MultiPoint') {
                for (let j = 0; j < rRange.geometry.geometries.length; j++) {
                    const rgeom = rRange.geometry.geometries[j].coordinates[0];
                    if (rRange.geometry.geometries[j].type === 'MultiPoint') lRange.geometry.geometries[i].coordinates.push(rgeom);
                }
            }
        }

        itps.splice(spliceIdx, 0, lRange);

    } else if (leftEnd !== rightEnd) {
        if (ladd) {
            lRange.properties['carmen:rtohn'][0] = null;
            lRange.properties['carmen:rfromhn'][0] = null;
            itps.splice(spliceIdx, 0, lRange);
        }
        if (radd) {
            spliceIdx = isStart ? rightEnd : rightEnd + 1;
            rRange.properties['carmen:ltohn'][0] = null;
            rRange.properties['carmen:lfromhn'][0] = null;
            itps.splice(spliceIdx, 0, rRange);
        }
    }
    return itps;
}
