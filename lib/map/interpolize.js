...
function buildExtRangeSide(itps, diff, isPrev, isLeft) {
    ...
    if ((isPrev && lfrom < lto) || (!isPrev && lfrom > lto)) {
        ...
        range.properties[toProp][0] = assignEdgeNumber(lfrom, extNumber);
        number = range.properties[toProp][0];
    } else {
        const extNumber = isPrev ? dropLow(lto, diff) : raiseHigh(lto, diff);
        range.properties[toProp][0] = assingExtendedNumber(lto, extNumber);
        range.properties[fromProp][0] = assignEdgeNumber(lto, extNumber);
        number = range.properties[fromProp][0];
    }

    //  When the address number has been raised/dropped
    if (range.properties[fromProp][0] !== undefined && range.properties[toProp][0] !== undefined) {
        hasNumberChanged = true;
        range = buildExtRangeGeometry(itps[idx], range, number, isPrev);
    } else return;
    if (!range) return;

    return { range, hasNumberChanged, idx };
}
...
/**
 * Creates the segment geometry for the extended range
 ...
 */
function buildExtRangeGeometry(itp, range, number, isPrev) {
    if (!itp || !itp.properties || !itp.properties['carmen:addressnumber']) return;

    const addressnumbers = itp.properties['carmen:addressnumber'][1].map((num) => parseInt(num));
    const numberIdx = isPrev ? addressnumbers.indexOf(number) : addressnumbers.lastIndexOf(number);
    if (numberIdx < 0) return;
    // Get the geometry point of the original number for the range side
    let numberGeom;
    for (let i = 0; i < range.geometry.geometries.length; i++) {
        if (range.geometry.geometries[i].type === 'MultiPoint') {
            numberGeom = range.geometry.geometries[i].coordinates[numberIdx];
            range.geometry.geometries[i].coordinates = [];
        }
    }

    // Calculate the lineString for range side
    for (let i = 0; i < range.geometry.geometries.length; i++) {
        if (range.geometry.geometries[i].type === 'LineString') {

            const stop = turf.point(numberGeom);
            // calculate line from start
            const startFromStart = turf.point(range.geometry.geometries[i].coordinates[0]);
            const lineFromStart = turf.lineSlice(startFromStart, stop, range.geometry.geometries[i]);
            // Calculate line from end
            const startFromEnd = turf.point(range.geometry.geometries[i].coordinates[range.geometry.geometries[i].coordinates.length - 1]);
            const lineFromEnd = turf.lineSlice(startFromEnd, stop, range.geometry.geometries[i]);

            // The extended segment is always smaller than the original segment
            if (turf.length(lineFromEnd) > turf.length(lineFromStart)) range.geometry.geometries[i].coordinates = lineFromStart.geometry.coordinates;
            else range.geometry.geometries[i].coordinates = lineFromEnd.geometry.coordinates;
        }
    }

    // TODO: Test existing calculation en rangeOptions since it already have calculated the distance.
