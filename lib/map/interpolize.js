 ...
 * @param  {object[]} options.distStart  sorted features for the left and right start
 * @param  {object[]} options.distEnd    sorted features for the left and right end
 * @param  {object}   options.parity     object with even/odd number information per left/right side
 * @param  {number}   options.leftside    0 or 1 for left or right side.
...
function generateInterpolationRange(options) { // Question, should this be singluar
    const result = {
        parityl: null,
        lstart: null,
        lend: null,
...
function checkInterpolationRanges(itps) {
    ...
    return itps.map((itp, i) => {
        ...
        const range = generateInterpolationRange(itp.rangeOptions); // TODO don't regenerate if no overlap
