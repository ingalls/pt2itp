 ...
 * @return {Segment[] | boolean} New clusters array
 */
function breakCluster(segs, id) {
    ...
    for (let i = 0; i < segs.length; i++) {
        if (!segs[i].number) {
            segDup[i] = false;
        } else if (Misc.hasDupAddressWithin(segs[i].number.map((prop) => prop.number), segs[i].address.geometry.coordinates)) {
            segDup[i] = true;
        } else {
            segDup[i] = false;
        }
    ...
    for (let seg_it = 0; seg_it < segs.length; seg_it++) {
        ...
        if (!breaks.length) {
            ...
            Array.prototype.push.apply(newSegs, _breakCluster(breaks, seg, dist));
        }
    }
...
// TODO rename
function _breakCluster(breaks, seg, points) {
    const dist = points;
    const newSegs = [];
