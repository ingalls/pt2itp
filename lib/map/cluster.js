...
class Cluster {
     ...
     */
    static detectCliffs(breaks, dist, distDelta) {
        let floating_min = 0;
        let floating_max = 0;
     ...
     * @param {Array[]} distDelta
     */
    static detectPeaks(breaks, dist, distDelta) {

        const avg_window = Math.min(Math.ceil(dist.length / 10), 10);

        const avg = this.movingAverageFilter(distDelta, avg_window);
    ...
    static movingAverageFilter(distDelta, avg_window) {
        let len = distDelta.length;
        const avg = [];
        let v = distDelta[0][1];
        avg.push(v);
        for (let i = 1; i < len; i++) {
            if (i >= avg_window) {
                v -= distDelta[i - avg_window][1];
            }
            v += distDelta[i][1];
            avg.push(v / i);
        }
